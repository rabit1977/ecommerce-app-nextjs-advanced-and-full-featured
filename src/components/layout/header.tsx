    ...
    <header className="sticky top-0 z-30 w-full border-b bg-white/80 backdrop-blur-sm dark:bg-slate-950/80 dark:border-slate-800">
      <div className="container mx-auto flex h-16 items-center justify-between px-4 gap-4">
        ...
        >
          ...
          <span className="text-xl font-bold hidden sm:inline dark:text-white">Electro</span>
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="hidden md:flex items-center gap-2">
            {user ? (
              <>
                <span className="text-sm font-medium dark:text-white">Hi, {user.name}</span>
                <Button variant="outline" size="sm" onClick={logout}>Logout</Button>
              </>
            ) : (
              <Button size="sm" onClick={() => navigateTo('/auth')}>Login</Button>
            )}
          </div>
